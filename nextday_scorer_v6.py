...
def load_mtf(sb):
    """Weekly trend + multi-timeframe state from momentum_mtf (mtf_scan.py).

    Recorded on each pick with ZERO weight — it does not change the score.
    It exists so the Journal and learner can measure whether picks that go
    against the weekly trend do worse. momentum_mtf is a one-row-per-stock
    snapshot refreshed at 8:30 PM, so a 6:40 PM run reads the previous
    night's weekly state (weekly states only change on Fridays)."""
    out = {}
    try:
        rows = page_select(lambda a, b: (
            sb.table("momentum_mtf").select("symbol,w_state,mtf_state,session_date")
              .order("symbol").range(a, b).execute().data), key="symbol")
        for r in rows:
            out[r["symbol"]] = r
        print("  momentum_mtf: %d symbols (weekly trend, weight 0)" % len(out))
    except Exception as e:
        print("  momentum_mtf read failed (%s) — no weekly badges" % str(e)[:70])
    return out


def wk_align(direction, w_state):
    if not w_state:
        return "unknown"
    if (direction == "bull" and w_state == "Up") or \
       (direction == "bear" and w_state == "Down"):
        return "with"
    if (direction == "bull" and w_state == "Down") or \
       (direction == "bear" and w_state == "Up"):
        return "against"
    return "flat"
...
def swing_levels(ehist, lookback=20):
    """Recent swing high / low from prev_high / prev_low already carried in
    the ema_filter history.

    Without this the only levels above a trigger were the 52-week high, the
    QM pivot and the last breakout price — all usually miles away. The
    result was that 8 of 9 picks scored full marks for "room to run", so the
    component added a flat 10 points to everyone and separated nothing.
    A 20-session high is a real level that price actually stalls at."""
    hi = [f(r.get("prev_high")) for r in ehist[:lookback]]
    lo = [f(r.get("prev_low")) for r in ehist[:lookback]]
    hi = [x for x in hi if x]
    lo = [x for x in lo if x]
    return (max(hi) if hi else None), (min(lo) if lo else None)


def nearest_level(e, m, ltp, direction, swing_hi=None, swing_lo=None):
...
def score_room(room_adr, wmax):
    if room_adr is None:          # clear sky, nothing in the way
        return wmax
    pts = band(room_adr, [(0.3, 0.0), (0.7, 3.0), (1.5, 6.0),
                          (2.5, 8.5), (float("inf"), 10.0)])
...
def evaluate(sym, ehist, m, sectors, earnings, orbhist, w,
    ...
    swing_hi, swing_lo = swing_levels(ehist)
...
def main():
    ...
    t0 = time.time()
    print("=" * 62)
    print(" TrueFlow Next Day WL v2 — scorer (rev 3, swing levels)")
    print("=" * 62)
    ...
    mtf = load_mtf(sb)
    for p in picks:
        r = mtf.get(p["symbol"]) or {}
        p["weekly_trend"] = r.get("w_state")
        p["mtf_state"] = r.get("mtf_state")
        p["wk_align"] = wk_align(p["direction"], r.get("w_state"))
        tag = {"with": "WK_WITH", "against": "WK_AGAINST",
               "flat": "WK_FLAT"}.get(p["wk_align"])
        if tag:
            p["badges"] = (p["badges"] + "," + tag) if p.get("badges") else tag
